...
exports.myLike = async(req, res) => { // 나의 찜하기 목록을 보여줌 -> product_no말고 진짜 product no에 해당하는 상품 정보를 띄워주기
    ...
    try{
        ...
        if (jwtResult){
            ...
            const product_result = await conn.query(sel_product, user_no)
...
exports.myCoupon = async(req, res) => {
    ...
    try{
        ...
        if(jwtResult) {
            ...
            const coupon_list = await conn.query(sel_coupons, user_no)
            if(coupon_list[0].length === 0) {
                res.status(200).json({'msg' : `사용가능한 쿠폰이 없습니다.`})
            }
